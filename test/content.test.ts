...
import { calcContentRange, serveHead, serveObject } from '../src/content.ts';
...
test('GET without a Range header returns 200 with the full length (regression: non-Range GET was 206)', async () => {
	...
	let response = await serveObject(bucket, new Request('http://x/file'), 'file');
...
test('GET with a byte range returns 206 with Content-Range', async () => {
	...
	let response = await serveObject(bucket, new Request('http://x/file', { headers: { Range: 'bytes=2-4' } }), 'file');
...
test('GET with a suffix range returns the tail (regression: suffix value handling)', async () => {
	...
	let response = await serveObject(bucket, new Request('http://x/file', { headers: { Range: 'bytes=-5' } }), 'file');
	assert.equal(response.status, 206);
	assert.equal(response.headers.get('Content-Range'), 'bytes 5-9/10');
});

test('HEAD returns status and headers without a body', async () => {
	let bucket = fakeBucket(() => objectBody({ offset: 2, length: 3 }));
	let response = await serveHead(bucket, new Request('http://x/file', { headers: { Range: 'bytes=2-4' } }), 'file');
	assert.equal(response.status, 206);
	assert.equal(response.headers.get('Content-Range'), 'bytes 2-4/10');
